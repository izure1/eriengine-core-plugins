...
export class ActorRun extends TypedEmitter<ActorRunEvent> {
  ...
  private routingAutomatic(): void {
    ...
    const xDistance = sign.x - current.x
    const yDistance = sign.y - current.y
    let xSpeed = getSmaller(Math.abs(xDistance), Math.abs(this.speed))
    let ySpeed = getSmaller(Math.abs(yDistance), Math.abs(this.speed))
